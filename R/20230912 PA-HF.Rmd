...
met4_labels <- map(
  ...
  ~mutate(
    .x,
    hr_ci=paste0(
      format(round(hr_corrected,  3), nsmall=3L), " (",
      format(round(lci_corrected, 3), nsmall=3L), " - ", 
      format(round(uci_corrected, 3), nsmall=3L), ")"
      )
    ) %>%
...
xaxisL <- c(
  rep(NA, n_expo+1),
  rep("Usual TPA, MET-hour/day", n_expo+1))

HRLabel <- c(
  "HR (95% CI) per 4 MET-hour/day\nusual TPA:", rep("HR (95% CI):", 4), 
  "HR (95% CI) per 4 MET-hour/day\nusual TPA:", rep("HR (95% CI):", 4))
