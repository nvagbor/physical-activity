...
setwd ("J:/Physical activity and HF/R codes")
...
plot_list <- map2(.x = stratified_met, 
                  .y = seq_along(stratified_met), 
                    ...
                    ~ ggplot(data = .x,  mapping = aes(x     = mean_age, 
                      ...
                      geom_segment(mapping = aes(x = 28, xend = 28, y = 5, yend = 40),size = 0.7, color = "black") + 
                      geom_segment(mapping = aes(x = 30, xend = 80, y = 4, yend = 4), size = 0.7, color = "black") +
                    ...
                    # Text for group means
                     ...
                     # Axis titles  
                     ylab( list("Mean physical activity, MET-hr (95% CI)", NULL)[[.y]] ) + 
...
plot <- ggarrange (plotlist = plot_list, 
 ...
 annotate_figure(plot, 
                 top = text_grob("Figure 1: Variations in age-specific levels of physical activity by sex and area\n", 
                                 color = "black", 
                                 face  = "bold", 
                                 size  = 16))
...
for (j in seq_along(exposures)) {
    ...
    RDR_report <-
        resurv_var_df %>%
           ...
           add_row(.before = 1, dilution_ratio = RDR_ci[[j]]) %>%
           rename(
...
FAR <- map(.x = list(m_final[[1]]$dataframe, 
                     m_final[[2]]$dataframe
                     ), 
           ~.x %>% 
             dplyr::mutate(mean_met = resurv_met_df$mean_met_resurvey)
           )
...
No difference from the main analysis
...
for (df in seq_along(data_expanded)) {
    ...
    map(
      ...
      ~Cox_shape_strength_data(
          ...
          exposure_var_cat = .x,
          expo_type_shape  = TRUE,
          adjustment_vars  = c(ses_vars, lifestyle_vars, "emphysema_bronchitis", "copd"), 
          print_model_info = FALSE) %>% 
        pluck(., 2)
      )
...
SetPage(orient ="LANDSCAPE", 
        ...
        suppress.date = TRUE,
        titlespace=0.07,
        footer="All analyses were stratified by age-at-risk, sex, and study area and adjusted for income, education, occupation, alcohol, smoking, sedentary time,\n ephysema, bronchitis and COPD. HF = Heart failure; PHD = Pulmonary heart disease; PA = Physical activity\n
        ",
        footer.cex = 1.2,
        footerspace=0.09,
        filestem="Figure test type Shape Plot PA and heart failure")
