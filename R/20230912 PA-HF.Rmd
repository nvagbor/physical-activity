...
```{r}
# Summarise mean temperatures by region, and month 
temp_mean_df <- 
  merged_data %>% 
  group_by(region, study_date_month) %>% 
  summarise(region_mean_temp=mean(mean_temp, na.rm=TRUE), .groups="drop") %>% 
  as.data.frame(.) 

# In the main dataset, imput missing value by region and month 
region_levels <- levels(merged_data$region)
merged_data$mean_temp_imputted <- NA
# Assign observed temperature values 
# You need to define this first, else it will be difficult to implement the loop 
...
for (r in region_levels){
  for (m in seq_len(12)){
    ...
    merged_data <- mutate(
      merged_data,
      mean_temp_imputted=case_when(
        region==r & study_date_month==m & is.na(mean_temp) ~ 
          filter(temp_mean_df, region==r, study_date_month==m) %>% 
          pull(region_mean_temp), # Assigned mean temp for that month 
        TRUE~mean_temp_imputted # Else assign measured temp
        )
      )
    } 
  }

# Replace rest of imputed temp with measured temp (for non-missing temp values)
merged_data <- mutate(
  merged_data, 
  mean_temp_imputted=case_when(is.na(mean_temp_imputted)~mean_temp, TRUE~mean_temp_imputted))
...
df_table1 <- filter(
  df_table1, 
  !grepl("_Other|_Middle.school.or.higher|_Primary|_10k.and.over|_No", exposure))

# Standardise age ---
df_table1_age <- make_standardised_ckb_data(
   dataset=merged_data, 
   numeric_vars="age",
...
ses_vars       <- c("h_income", "edu", "agric_factory", "mean_temp_imputted")
...
for (j in seq_along(exposures)){
    ...
    ## Summary data for Peto RDR
    summary_table[[j]] <- select(
      resurv_var_df, baseline_group, contains("mean_var")) %>% 
      mutate(
        exposure=c("Total PA", "Occupational PA", "Non-occupational PA")[j], 
        rdr=RDR_peto[[j]],
        across(where(is.numeric), ~format(round(.x, 2), nsmall=2))
      ) %>% 
      arrange(baseline_group)
...
resurv_PA <- map2(
  .x=resurv_merged_df, 
  .y=seq_along(resurv_merged_df),
  ~select(.x, csid, contains(exposures[.y]), resurv_ave) %>% 
    rename(!!paste0("resurv_ave_", exposures)[.y]:=resurv_ave)
)

# Join resurvey datasets for sensitivity analysis by PA types --
resurvey_sensitivity <- 
  inner_join(resurv_PA[[1]], resurv_PA[[2]], by="csid") %>% 
  inner_join(., resurv_PA[[3]], by="csid")
...
map_dfr(
  ...
  ~add_table_header(
    data=.x, 
    group_var=exposure, 
    group_level_var=baseline_group) 
    ) %>% 
  mutate(across(everything(), ~case_when(is.na(.x)~"", TRUE~.x))) %>% 
  write.csv(., file="Q5 means for PAs.csv")
...
FARFlat <- flatten(FAR)
ModelContFlat <- flatten(model_cont)
EName <- rep(names(expos), 2)
...
HRLabel <- map2(
  ...
  ~mutate(
    ...
    exposure=c(
      "Per 4 MET-hour/day: ", 
      paste0("Per 1 SD (", format(round(sd(data_expanded[[1]][[.y]]), 1), nsmall=1), "): ")), # For labelling
...
HRHeader <- c(
  "HR (95% CI) per higher usual PA", rep("", 2), 
  "HR (95% CI) per higher usual PA", rep("", 2))
...
for (j in seq_along(plotdata)){
	...
	LinearPlot(plotdata[[j]], 
	           ...
	           xlim              = if(j%in%c(1, 4)){c(10,30)}else{c(0,30)},  
	           xticks            = if(j%in%c(1, 4)){seq(10,30, 5)}else{seq(0,30, 5)}, 
	           xticklabs         = if(j%in%c(1, 4)){
	                                 c("10", "", "20", "", "30")}else{
	                                   c("0", "", "10", "", "20", "", "30")} ,
	           ylim              = c(0.25, 2), 
	           yticks            = c(0.25, 0.5, 1, 2), 
	           margins           = c(3,5,5,2),  # c(bottom, left, top, right)
	           xticklabs.cex     = 0.9, 
	           yticklabs.cex     = 0.9,
	           PointLabelsTop    = NULL,
	...
	AxisLabels(
	  ...
	  xlab=c(
	    ...
	    "",
	    "", 
	    "Usual total PA, MET-hour/day", 
	    "Usual occupational PA, MET-hour/day",
	    "Usual non-occupational PA, MET-hour/day")[j],
	  mainfont = 1
	  )
	...
	text(x=if(j%in%c(1, 4)){5}else{-5}, y=log(2.4), labels=headers[j], adj = c(0,0), font=2, cex = 1.2)
	text(x=if(j%in%c(1, 4)){10}else{0}, y=log(1.9), labels=subheaders[j], adj = c(0,0), font=2, cex = 1)
	
	# Add labels for HR 
	if(j%in%c(1, 4)){x.pos=10}else{x.pos=0}
	text(x=x.pos, y=log(0.305), font=2, cex=0.9, adj=c(0, 0), labels=HRHeader[j])
	text(x=x.pos, y=log(0.25), font=1, cex=0.9, adj=c(0, 0), labels=paste0(HRLabel[[j]], collapse="\n"))
 }
...
SetPage(orient          = "LANDSCAPE", 
        ...
        type            = type, 
        png_bg          = "white",
        filestem        = "Supplementary eFigure 1 - Age-sex-specific analysis", 
        page_dpi        = 600,
        page_pointsize  = 9,
        page_height     = 5,
        page_width      = 7,
...
SetPage(orient          = "PORTRAIT", 
        perpage         = 1, 
        type            = type, 
        filestem        = "Figure 3 - Subgroup analysis", 
        page_dpi        = 600,
        page_pointsize  = 8,
        page_height     = 9,
        page_width      = 7.5,
...
for (df in seq_along(region_subgroup)){
  ...
  region_forest_temp[[df]] <- mutate(
    ...
    mutate(
      term=case_when(
        term=="Urban"~"All urban", 
        term=="Rural"~"All rural", 
        TRUE~term), 
      IsDiamond=case_when(grepl("All", term)~1, TRUE~0), 
      FillColour=case_when(grepl("All", term)~0, TRUE~1))
...
SetPage(orient          = "LANDSCAPE", 
        perpage         = 1, 
        type            = type, 
        filestem        = "Supplementary eFigure 2 - Region-specific analysis", 
        page_dpi        = 600,
        page_pointsize  = 9,
        page_height     = 6,
        page_width      = 7,
...
sequential_adjust_list <- list(
  "minimal adjustment"=c("h_income"), 
  "ses"=list(
    "var_names"=c("edu", "agric_factory", "mean_temp_imputted"), 
    "var_labels"=c(
          "     + Education", 
          "     + Occupation", 
          "     + Temperature")
...
for (df in seq_along(data_expanded)){
  ...
  # Combine datasets --
  df_ses[[df]] <- mutate(bind_rows(list_ses), name_group="Demographic and SES factors") 
...
for (df in seq_along(data_expanded)) {
  ...
  for (l in seq_along(factors_ls)){
    if(l > 1){
      ...
      } else {adjust_ls <- c(sequential_adjust_list$`minimal adjustment`, sequential_adjust_list$ses$var_names)}
    
    list_lifestyle[[l]] <- lrtest_change_chi2(
      dataset = data_expanded[[df]], 
      main_expo_var  = "met4", 
  ...
  # Combine datasets --
  df_lifestyle[[df]] <- mutate(bind_rows(list_lifestyle), name_group="Lifestyle factors")
...
df_minimal_temp <- map(
  .x=df_minimal, 
  ~select(.x, -c(p_value_before, confounder_adjust_before, adjustment)) %>% 
...
df_confounding <- map2(
  ...
  ~bind_rows(.x, .y) %>% 
    mutate(
      est_corrected=estimate/RDR_peto[[1]],   # Correct estimates and SE for regression dilution
      se_corrected=std.error/RDR_peto[[1]],
      chi2_after=format(round(chi2_after, 1), nsmall = 1, scientific = FALSE),
      minimal_estimate=as.numeric(.data[["est_corrected"]][1]),
      cum_hr_delta=((est_corrected-minimal_estimate)/est_corrected)*100, 
      hr_change100=paste0(format(round(cum_hr_delta, 1), nsmall=1), "%"),
      IsDiamond=0, 
      FillColour=1
      ) %>%  
...
df_confounding <- map(
  .x=df_confounding,
  ~mutate(.x, name_group=ifelse(is.na(name_group), Heading, name_group)) %>% 
    add_table_header(
      data=.,
      ...
      extra_space=FALSE) %>%
    slice(-1) %>%
    mutate(Show=ifelse(is.na(FillColour), NA, 1)) %>%
    as.data.frame(.)
  )
...
# set up an object that stores the forest and associated metadata
forest2 <- FormatForest(
     rawdata=df_confounding[[2]] %>% 
       rename("IsDiamond2"=IsDiamond, 
              "FillColour2"=FillColour, 
              "Show2"=Show) ,  
     forest.n    = 2, 
     EstimateCol = "est_corrected",  
...
# find labels to put in the left most column
left.labels <- convertUnicode(as.character(df_confounding[[1]]$Heading))

# what other columns should be plotted
other.cols  <- "chi2_after"

# find column headings
print.headings <- parseColHeadings(df_confounding[[1]]$ColHeadings, rd=df_confounding[[1]])
...
SetPage(orient          = "LANDSCAPE",
        ...
        perpage         = 1, 
        type            = type, 
        filestem        = "Figure 2 - Serial adjustment for confounding", 
        page_dpi        = 600,
        page_pointsize  = 10,
        page_height     = 6,
        page_width      = 8,
...
# Set page margins: format is c(bottom, left, top, right)
par(mar=c(4.14537797266251, 0.5, 4.14537797266251, 0.5))
...
for (df in seq_along(df_confounding)) {
...
## Panel titles --
  ...
  x.positions <- c(xaxmax1 + 19.5, xaxmax2 + 20.5)
  ...
  if(is.null(blank.rows)){
    ...
    text(
     x=x.positions[df], 
     y= locs[[df]]$YLocs, 
     labels=convertUnicode(as.character(df_confounding[[df]] [,other.cols[1]])), 
     font=ifelse(forest1$IsDiamond, 2, 1),, 
     adj=1, 
     cex=1)
    ...
    text(
     x=x.positions[df], 
     y= locs[[df]]$YLocs, 
     labels=convertUnicode(as.character(df_confounding[[df]] [,other.cols[1]]))[-blank.rows], 
     font=ifelse(forest1$IsDiamond, 2, 1),, 
     adj=1, 
     cex=1)
...
Mediation analysis will investigate the impact of further adjustments for waist circumference, BMI, diabetes, and hypertension on the shape and strength of associations.
...
expos       <- c(
  "met_q5_sex", "met4", "metsd", "met",
  "met_work_q5_sex", "met_work4", "met_worksd", "met_work", 
  "met_nonwork_q5_sex", "met_nonwork", "met_nonwork4", "met_nonworksd")
...
subset_vars <- subset_vars[-which(subset_vars=="I(sed_time_hr^2)")]
  
data_expanded <- map(.x = data_expanded, ~select(.x, all_of(subset_vars)))

```


## Further adjustments for mediators

- Do further adjustments for BMI, WC, hypertension, and diabetes.
- Add quadratic terms to account for non-linearity.

```{r}

main_adjust <- c(ses_vars, lifestyle_vars) 
mediators <- list(
  "main_model"    = NULL,
  "bmi_calc"      = c("bmi_calc", "I(bmi_calc^2)"), 
  "waist_circ_cm" = c("bmi_calc", "I(bmi_calc^2)", "waist_circ_cm", "I(waist_circ_cm^2)"), 
  "diabetes"      = c("bmi_calc", "I(bmi_calc^2)", "waist_circ_cm", "I(waist_circ_cm^2)", "diabetes"), 
  "HTN"           = c("bmi_calc", "I(bmi_calc^2)", "waist_circ_cm", "I(waist_circ_cm^2)", "diabetes", "HTN"), 
  "heart_rate_mean" = c("bmi_calc", "I(bmi_calc^2)", "waist_circ_cm", "I(waist_circ_cm^2)", "diabetes", "HTN", "heart_rate_mean", "I(heart_rate_mean^2)")
  )

# Create lists to hold data 
shape_mediation    <- list()
met_cont_mediation <- list()
do_print <- FALSE

for (O in seq_along(data_expanded)) { 
  
  ONamei    <- c("HF", "PHD")[O]
  # adjust   <- NULL  
  # adjust   <- c(main_adjust, mediators[[O]])

# Shape --
shape_mediation[[ONamei]] <- map(
  .x=names(mediators), 
  ~Cox_shape_strength_data(
     dataset=data_expanded[[O]], 
     exposure_var_cat="met_q5_sex", 
     expo_type_shape=TRUE,
     correct_regression_dilution=FALSE,
     adjustment_vars=c(main_adjust, mediators[[.x]]),
     stratify_vars=strata_vars,
     print_model_info=do_print
     ) %>% 
    pluck(., 2) %>%  
    mutate(analysis=paste0("Further adjustment for ", paste0(mediators[[.x]], collapse=", "))) %>% 
    inner_join(
      ., 
      select(resurvey_var_data[[1]], -expo_name), 
      by="exposure"
    ),
  .progress=TRUE
  ) %>% 
  setNames(names(mediators))

# Met4 --
met_cont_mediation[[ONamei]] <- map(
  .x=names(mediators),
  ~Cox_shape_strength_data(
     dataset=data_expanded[[O]], 
...
# Calculate percent change in logHR 
hr_main <- map(
  .x=met_cont_mediation, 
  ~slice(.x[["main_model"]], 1) %>% pull(est_corrected)) %>% 
  setNames(c("HF", "PHD"))

for(M in names(met_cont_mediation)){
  
  met_cont_mediation[[M]] <- map(
    .x=met_cont_mediation[[M]],
    ~mutate(
      .x,
      delta_logHR=(hr_main[[M]]-est_corrected)/hr_main[[M]]*100, 
      delta_logHR=paste0(format(round(delta_logHR, 1), nsmall=1), "%")
    )
  )
}

# Simplify lists and prepare for plotting 
shape_flat    <- flatten(shape_mediation)
strength_flat <- flatten(met_cont_mediation)

```

## Plot shape (mediators) 
...
events_sum <- map(
  .x=shape_flat, 
  ~paste0("(n=", prettyNum(sum(.x$n), big.mark=",", scientific=FALSE), ")"))
...
met4_labels <- map(
  ...
  ~mutate(
    ...
    select(term, hr_ci, delta_logHR, analysis)
  )
...
xaxisL <- c(
  rep(NA, n_mediators),
  rep("Usual TPA, MET-hour/day", n_mediators))

HRLabel <- c(
  "HR (95% CI) per 4 MET-hour/day\nusual TPA:", rep("HR (95% CI):", n_mediators), 
  "HR (95% CI) per 4 MET-hour/day\nusual TPA:", rep("HR (95% CI):", n_mediators))

DeltaLabel <- "Reduction in logHR:"

# set up the Jasper page
type <- "PNG"
SetPage(
  orient         = "LANDSCAPE",
  # perpage        = length(plotdata),
  nlong          = length(plotdata)/2,
  nshort         = 2,
  type           = type, 
  png_bg         = "white",
  page_width     = 14,
  ...
  footerspace    = 0.02,
  footer.cex     = 1.5,
  filestem       = "Supplementary eFigure 3 - Mediation analysis")
...
for (j in seq_along(plotdata)){
	...
	# add axis labels
	AxisLabels(ylab=yaxisL[[j]], xlab=xaxisL[[j]], mainfont=1)
	par(xpd=NA)
	
	# add the column heading
	if(j <= n_mediators) {
	 text(x=20, y=log(2.8), labels=headers[j], font=2, cex=1.2)
	}
	 # text(x=20,  y=log(1.9), labels=events_sum[[j]], font=1, cex=1)
	 text(x=10,  y=log(2.2), labels=outcome[[j]], font=2, adj=c(0,0), cex=1.2)
	
	# Add labels for HR 
	text(x=10, y=log(0.32), font=1, cex=0.9, adj=c(0, 0), labels=paste(HRLabel[j], met4_labels[[j]]$hr_ci))
	text(x=10, y=log(0.28), font=1, cex=0.9, adj=c(0, 0), labels=paste(DeltaLabel, met4_labels[[j]]$delta_logHR))
	}
...
## Import resurvey data and join 

```{r}
data_expanded <- map(
  .x=data_expanded,
  ~left_join(.x, select(resurvey_sensitivity, csid, contains("resurv_ave")), by="csid"))
...
# Define HF and PHD co-occurrence -- 
data_expanded <- map(
  .x = data_expanded, 
  ~mutate(.x, co_hf_phd=ifelse(HF_combined_ep==1 & PHD_combined_ep==1, "Yes", "No")))

# Create list of exclusions 
exclusions <- list(
  "fu"=list(
    "expression"="lex.dur>2", 
    "label"="A: Excluding the first two years of follow-up"), 
  "poor health"=list(
    "expression"="lex.dur>2 & poor_health=='No'", 
    "label"="B: (A) + excluding poor self-rated health"),
  "CLD"=list(
    "expression"="lex.dur>2 & poor_health=='No' & CLD=='No'", 
    "label"="C: (B) + excluding shortness of breath on walking"),
  "SOB"=list(
    "expression"="lex.dur>2 & poor_health=='No' & CLD=='No' & SOB_walking=='No'", 
    "label"="D: (C) + excluding chronic respiratory disease"),
  "HF_PHD"=list(
    "expression"="lex.dur>2 & poor_health=='No' & CLD=='No' & SOB_walking=='No' & co_hf_phd=='No'", 
    "label"="E: (D) + excluding co-occurring HF and PHD")) 

# Exposure and Outcome Lists --
expos <- list(
    met=list(
      "q5"="met_q5_sex", 
      "cont"=c("met4", "metsd"), 
      "resurvey_mean"="resurv_ave_met", 
      "adjustment"=c(ses_vars, lifestyle_vars)
      ),  
    met_work=list(
      "q5"="met_work_q5_sex", 
      "cont"=c("met_work4", "met_worksd"), 
      "resurvey_mean"="resurv_ave_met_work", 
      "adjustment"=c(ses_vars, lifestyle_vars, "met_nonwork")
      ), 
    met_nonwork=list(
      "q5"="met_nonwork_q5_sex", 
      "cont"=c("met_nonwork4", "met_nonworksd"), 
      "resurvey_mean"="resurv_ave_met_nonwork", 
      "adjustment"=c(ses_vars, lifestyle_vars, "met_work")
      )
    )


outcome_name <- c("hf", "phd")

# Results Lists--
shape_list <- list() 
strength_list <- list()

for (gg in seq_along(expos)) {
  #Loop over exposures --
  expos_name_temp <- names(expos)[gg]
  expos_q5_temp   <- expos[[expos_name_temp]]$q5
  expos_cont4_temp <- expos[[expos_name_temp]]$cont[1]
  expos_contsd_temp <- expos[[expos_name_temp]]$cont[2]
  resurvey_mean_temp <- expos[[expos_name_temp]]$resurvey_mean
  adjustment_temp <- expos[[expos_name_temp]]$adjustment
  
  for(kk in seq_along(exclusions)){
   
    exclude_temp <- names(exclusions)[kk]
    
    # Start message --- 
    cat("Running analysis for ", exclude_temp, "\n")
    
    # Perform exclusion -- 
    dat_temp <- map(
      .x=data_expanded, 
      ~filter(.x, eval(parse(text=exclusions[[kk]]$expression))))
    ...
    dat_temp <- map(
      ...
      ~group_by(.x, sex) %>% 
        ...
        map(
          ~mutate(
             .x, 
             "{{expos_q5_temp}}":=factor(quantcut(.data[[expos_name_temp]], 5), labels = paste0("Q", 1:5)),
             "{{expos_cont4_temp}}":=.data[[expos_name_temp]]/4, 
             "{{expos_contsd_temp}}":=scale(.data[[expos_name_temp]]))
           ) %>%
        map_dfr( ~rbind(.x))
      )
    
    # Recalculate the quintile-specific resurvey --
    met_means <- map(
      .x=dat_temp, 
      ~reframe(.x, resurv_mean=mean(.data[[resurvey_mean_temp]], na.rm=TRUE), .by=all_of(expos_q5_temp)) %>% 
        mutate(exposure=as.character(.data[[expos_q5_temp]])) %>% 
        select(exposure, resurv_mean)
      )
  
    # Shape ---
    shape_fu2 <- map2(
      .x=dat_temp, 
      .y=seq_along(dat_temp),
      ~Cox_shape_strength_data(
        dataset                     = .x, 
        exposure_var_cat            = expos_q5_temp, 
        expo_type_shape             = TRUE,
        correct_regression_dilution = FALSE,
        adjustment_vars             = adjustment_temp,  
        stratify_vars               = stratify,
        print_model_info            = FALSE) %>% 
       pluck(., 2) %>% 
        inner_join(., met_means[[.y]], by="exposure") %>% 
        mutate(
          analysis=exclusions[[kk]]$label, 
          resurv_mean=format(round(resurv_mean, 2), nsmall=2),
          HR=exp(estimate), 
          LCI=exp(estimate-1.96*std.error), 
          UCI=exp(estimate+1.96*std.error),
          HRCI=paste0(
            format(round(HR,  2), nsmall=2L), " (",
            format(round(LCI, 2), nsmall=2L), " - ", 
            format(round(UCI, 2), nsmall=2L), ")"), 
          p.value=NA_character_
          ) %>% 
        select(analysis, exposure, resurv_mean, n, HRCI,  p.value), 
      .progress=TRUE) %>% 
      setNames(outcome_name)
  
    shape_list[[expos_name_temp]][[exclude_temp]] <- shape_fu2 
    
    # Strength --
    cont_fu2 <- list()
    
    for (o in seq_along(outcome_name)){
      
      sd_var <- format(round(sd(dat_temp[[o]][[expos_name_temp]], na.rm=TRUE), 1), nsmall=1)
      
      cont_fu2[[ outcome_name[o] ]] <- map2_dfr(
         .x=expos[[expos_name_temp]]$cont,
         .y=c("Per 4 MET-hour/day", paste0("Per 1 SD (", sd_var, ")")),
         ~Cox_shape_strength_data(
             dataset                     = dat_temp[[o]], 
             exposure_var_cont           = .x, 
             expo_type_continuous        = TRUE,
             correct_regression_dilution = TRUE, 
             regression_dilution_ratio   = RDR_peto[[gg]],
             adjustment_vars             = adjustment_temp, 
             stratify_vars               = stratify, 
             print_model_info            = FALSE) %>% 
           pluck(., 2) %>%
           mutate(
             analysis=exclusions[[kk]]$label,
             n=sum(dat_temp[[o]]$lex.Xst, na.rm=TRUE),
             resurv_mean=NA_character_,
             term=.y,
             HR=exp(estimate), 
             LCI=exp(estimate-1.96*std.error), 
             UCI=exp(estimate+1.96*std.error),
             HRCI=paste0(
               ...
               format(round(LCI, 2), nsmall=2L), " - ", 
               format(round(UCI, 2), nsmall=2L), ")"), 
             p.value=case_when(p.value<0.0001~"<0.0001", TRUE~format(round(p.value, 4), nsmall=4))
             ) %>% 
           select(analysis, exposure=term, n, HRCI, resurv_mean, p.value), 
         .progress=TRUE
         )
      }
    
    strength_list[[expos_name_temp]][[exclude_temp]] <- cont_fu2
    cat("Finished analysis for", exclude_temp, "\n \n")
  }
}
...
for (gg in seq_along(expos)){
  expos_temp <- names(expos)[gg]
  
  for(Q in names(exclusions)){
  
  # Merge shape and stregnth data
  sensitivity_temp[[expos_temp]][[Q]] <- map2(
    .x=shape_list[[expos_temp]][[Q]], 
    .y=seq_along(strength_list[[expos_temp]][[Q]]), 
    ~rbind(.x, strength_list[[expos_temp]][[Q]][[.y]]) %>% 
      mutate(Outcome=c("Heart Failure", "Pulmonary Heart Disease")[.y])) 
...
# Rearrange datasets by outcome x exclusion
sensitivity <- list()

for (gg in seq_along(expos)){
  expos_temp <- names(expos)[gg]
  
  for(SS in outcome_name){
    
    sensitivity[[expos_temp]][[SS]] <- map_dfr(
        .x=sensitivity_temp[[expos_temp]],
        ~mutate(
          .x[[SS]], 
          HRCI=case_when(grepl(pattern="NA", HRCI)~"Reference", TRUE~HRCI), 
          exposure=case_when(
            !is.na(resurv_mean)~paste0(exposure, " (", resurv_mean, ")"), TRUE~exposure)
          )) %>% 
      select(-resurv_mean) %>% 
      # Add table header for reporting 
      add_table_header(
        data=., 
        group_var=analysis, 
        group_level_var=exposure
      ) %>% 
      mutate(across(.cols=everything(), ~case_when(is.na(.x)~"", TRUE~as.character(.x))))
  }
}
...
for (gg in seq_along(expos)){
  expos_temp <- c("TPA ", "OPA ", "Non-OPA")[gg]
  
  walk2(
    .x=sensitivity[[gg]], 
    .y=c(" HF", " PHD"),
    ~write.csv(.x, file=paste0("Sensitivity analysis for ", expos_temp, .y, ".csv"))
  )
