...
HRLabel <- map(
  ...
  ~pluck(.x, "met") %>% 
    mutate(
      exposure=c(
        "Per 4 MET-hour/day: ", 
        paste0("Per 1 SD (", round(sd(data_expanded[[1]]$met), 1), "): ")), # For labelling
...
SetPage(orient="LANDSCAPE", 
        ...
        footer.cex=1.2,
        footerspace=0.03,
        filestem="Figure 1 - Shape Plot PA")
...
for (j in seq_along(plotdata)){
	...
	# Add labels for HR 
	text(x=10, y=log(0.305), font=2, cex=0.9, adj=c(0, 0), labels="HR (95% CI) per higher usual TPA")
	text(x=10, y=log(0.27), font=1, cex=0.9, adj=c(0, 0), labels=paste0(HRLabel[[j]], collapse="\n"))
...
sdPA <- map_chr(
  ...
  ~format(round(sd(data_expanded[[1]][[.x]], na.rm=TRUE), 1), nsmall=1))
...
HRLabTypes <- map(
  ...
  map2(
    ...
    ~mutate(
      ...
      exposure=c(
        "Per 4 MET-hour/day: ", 
        paste0("Per 1 SD (", .y, "): ")), # For labelling
      hr_ci=paste0(format(round(hr_corrected, 2), nsmall=2),  " (",
                   format(round(lci_corrected, 2), nsmall=2), " - ", 
                   format(round(uci_corrected, 2), nsmall=2), ")"), 
      label=paste0(exposure, hr_ci)) %>% 
...
for (j in seq_along(plotdata)){
	...
	AxisLabels(
	  ...
	  xlab=c("Usual occupational PA, MET-hour/day",
	         "Usual occupational PA, MET-hour/day", 
	         "Usual non-occupational PA, MET-hour/day", 
	...
	# Add labels for HR 
	text(x=0, y=log(0.305), font=2, cex=0.9, adj=c(0, 0), labels="HR (95% CI) per higher usual PA")
	text(x=0, y=log(0.25), font=1, cex=0.9, adj=c(0, 0), labels=paste0(HRLabTypes[[j]], collapse="\n"))
 }
...
data_expanded <- map(
  .x=data_expanded,
  ~left_join(.x, resurv_merged_met %>% select(csid, resurv_ave), by="csid"))
...
exclusions <- list(
  "fu"=list("expression"="lex.dur>2", "label"="A: Excluding the first two years of follow-up"), 
  "poor health"=list("expression"="lex.dur>2 & poor_health=='No'", "label"="B: (A) + excluding poor self-rated health"),
  "CLD"=list("expression"="lex.dur>2 & poor_health=='No' & CLD=='No'", "label"="C: (B) + excluding shortness of breath on walking"),
  "SOB"=list("expression"="lex.dur>2 & poor_health=='No' & CLD=='No' & SOB_walking=='No'", "label"="D: (C) + excluding chronic respiratory disease"),
  "HF_PHD"=list("expression"="lex.dur>2 & poor_health=='No' & CLD=='No' & SOB_walking=='No' & co_hf_phd=='No'", "label"="E: (D) + excluding co-occurring HF and PHD")) 
...
for(kk in seq_along(exclusions)){
  ...
  met_means <- map(
    .x=dat_temp, 
    ~reframe(.x, resurv_mean=mean(resurv_ave, na.rm=TRUE), .by="met_q5_sex") %>% 
      mutate(exposure=as.character(met_q5_sex)) %>% 
  ...
  shape_list[[exclude_temp]] <- shape_fu2 
  ...
  for (o in seq_along(outcome_name)){
    ...
    sd_var <- format(round(sd(dat_temp[[o]]$met, na.rm=TRUE), 1), nsmall=1)
  ...
  strength_list[[exclude_temp]] <- cont_fu2
...
for(Q in names(exclusions)){
  ...
  sensitivity_temp[[Q]] <- map2(
    .x=shape_list[[Q]], 
    .y=seq_along(strength_list[[Q]]), 
    ~rbind(.x, strength_list[[Q]][[.y]]) %>% 
...
walk2(
  .x=sensitivity, 
  .y=c("HF", "PHD"),
  ~write.csv(.x, file=paste0("Sensitivity analysis for ", .y, ".csv"))
)
